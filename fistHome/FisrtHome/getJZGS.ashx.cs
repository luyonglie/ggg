...
namespace FisrtHome
{
    ...
    /// getJZGS 的摘要说明
    /// </summary>
    public class getJZGS : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string where = "";
            var qu = context.Request.Params["qu"];
            var qubm = context.Request.Params["qubm"]; 
            var jd = context.Request.Params["jd"];
            if (!string.IsNullOrWhiteSpace(jd))
            {
                where = $"and CorpID='{jd}' ";
            }
            else if (!string.IsNullOrWhiteSpace(qubm))
            {
                where = $"and CorpID='{qubm}' ";
            }
            ...
            try
            {
                ...
                sqlCommand.Parameters.Add(new SqlParameter("table", "T_R_Note_Jzgs"));
